...
from DRN_LwU.model.common import DownBlock
from DRN_LwU.model import drn

# torch.cuda.set_device(2)
def dataparallel(model, gpu_list=[0,1]):
    ngpus = len(gpu_list)
...
class Model(nn.Module):
    def __init__(self, opt, ckp):
        ...
        if not opt.cpu and opt.n_GPUs > 1:
            self.model = nn.DataParallel(self.model, [2,3])
            self.dual_models = dataparallel(self.dual_models, [2,3])
    ...
    def save(self, path, epoch, is_best=False):
        ...
        torch.save(
            target.state_dict(), 
            os.path.join(path, 'model', 'model_'+str(epoch)+'.pt')
        )
        if is_best:
            torch.save(
                target.state_dict(),
        ...
        torch.save(
            dual_models,
            os.path.join(path, 'model', 'dual_model_'+str(epoch)+'.pt')
        )
        if is_best:
            torch.save(
                dual_models,
