...
from DRN_LwU import optimizer_lib
from DRN_LwU import optimizer_adam
...
def make_optimizer(opt, my_model):
    ...
    optimizer_function = optimizer_adam.local_adam
    kwargs = {
    ...
    return optimizer_function(trainable, my_model.model.reg_lambda, **kwargs)

def make_base_optimizer(opt, my_model):
    trainable = filter(lambda x: x.requires_grad, my_model.parameters())
    optimizer_function = optim.SGD
    return optimizer_function(trainable, opt.lr, weight_decay=opt.weight_decay)

def make_base_dual_optimizer(opt, dual_models):
    dual_optimizers = []
    for dual_model in dual_models:
        temp_dual_optim = optimizer_lib.optim.SGD(
...
def make_dual_optimizer(opt, dual_models):
    ...
    for dual_model in dual_models:
        ...
        temp_dual_optim = optimizer_adam.local_adam(
            params=dual_model.parameters(),
            reg_lambda=dual_model.reg_lambda,
            lr = opt.lr, 
            betas = (opt.beta1, opt.beta2),
            eps = opt.epsilon,
            weight_decay=opt.weight_decay)
        '''
        temp_dual_optim = optimizer_lib.local_sgd(
            params=dual_model.parameters(),
            reg_lambda=dual_model.reg_lambda,
            lr = opt.lr,
            momentum=opt.beta1, 
