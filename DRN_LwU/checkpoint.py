...
class Checkpoint():
    def __init__(self, opt):
        ...
        if opt.save == '.': opt.save = './DRN_MAS/newexperiment/EXP/' + now
        self.dir = opt.save
    ...
    def save(self, trainer, epoch, is_best=False):
        ...
        torch.save(self.log, os.path.join(self.dir, str(epoch)+'_psnr_log.pt'))
        torch.save(
            trainer.optimizer.state_dict(),
            os.path.join(self.dir, str(epoch)+'_optimizer.pt')
        )
        dual_optimizers = {}
        ...
        torch.save(
            dual_optimizers,
            os.path.join(self.dir, str(epoch)+'_dual_optimizers.pt')
        )
    ...
    def plot_psnr(self, epoch):
        # axis = np.linspace(100, epoch, int(epoch/100))
        label = 'SR on {}'.format(self.opt.data_test)
        fig = plt.figure()
        plt.title(label)
        for idx_scale, scale in enumerate([self.opt.scale[0]]):
            axis = np.linspace(1, epoch, len(self.log[:, idx_scale].numpy()))
            plt.plot(
                axis,
